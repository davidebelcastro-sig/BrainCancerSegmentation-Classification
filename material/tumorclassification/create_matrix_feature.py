...
def calculate_characteristics(contour, original_image,tipo):
    ...
    mean_intensity_gradient = np.mean(intensity_gradient)
     ...
     # Calcola i Local Binary Patterns (LBP)
    ...
    with open('./training_tumor_feature.csv', 'a') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow([area, perimeter, solidity, aspect_ratio, eccentricity, hu_moment1, hu_moment2, hu_moment3, hu_moment4, hu_moment5, hu_moment6, hu_moment7, mean_intensity, mean_intensity_gradient, spectral_ratio,
                         texture_features[0], texture_features[1], texture_features[2], texture_features[3], texture_features[4], texture_features[5], texture_features[6], texture_features[7], texture_features[8], texture_features[9], texture_features[10], texture_features[11], texture_features[12], tipo])
...
def run(dir):
    for file in os.listdir(dir):
            ...
            contorno = contours[0]
            calculate_characteristics(contorno,img,"meningioma")
...
def initial():
      with open('./training_tumor_feature.csv', 'a') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['area', 'perimeter', 'solidity', 'aspect_ratio', 'eccentricity', 'hu_moment1', 'hu_moment2', 'hu_moment3', 'hu_moment4', 'hu_moment5', 'hu_moment6', 'hu_moment7', 'mean_intensity', 'mean_intensity_gradient', 'spectral_ratio',
                         'texture1', 'texture2', 'texture3', 'texture4', 'texture5', 'texture6', 'texture7', 'texture8', 'texture9', 'texture10', 'texture11', 'texture12', 'texture13', 'tipo'])     
